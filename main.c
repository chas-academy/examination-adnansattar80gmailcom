...
void calculate_averages(int scores[5][13], double averages[5], char names[5][11]) {
    for (int i = 0; i < 5; i++) {
        int sum = 0;
        for (int j = 0; j < 13; j++) {
            sum += scores[i][j];
...
void find_highest_average(double averages[5], char names[5][11]) {
    ...
    printf("\nHighest average score is %.2f for student %s\n", highest, names[index]);
}
...
int main() {
    ...
    printf("Enter name and scores for each student (Name p1 p2 p3 p4 ... p13):\n");
    ...
    printf("\nAverage scores:\n");
    double averages[5];
    calculate_averages(scores, averages, names);
